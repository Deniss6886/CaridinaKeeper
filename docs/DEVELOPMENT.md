...
- `npm run test:e2e` – production build, isolated preview server, complete Chromium desktop/mobile journeys, and axe checks.
...
5. From a fresh clone without `node_modules`, run `npm ci`, `npm run check`, `npm run test:e2e`, and `npm run security:audit`.
